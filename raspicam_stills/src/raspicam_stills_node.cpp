...
image_transport::Publisher pub_rgb_image;
...
static void camera_buffer_callback(MMAL_PORT_T *port, MMAL_BUFFER_HEADER_T *buffer)
{
   ...
   if (pData)
   {
      ...
      if (bytes_to_write && pData->file_handle)
      {
         ...
         /// Padded to be a multiple of 16, as per:
         /// http://stackoverflow.com/questions/20674415/extra-bytes-on-the-end-of-yuv-buffer-raspberrypi
         if(buffer->length==2592 * (122*16) * 3){ //15178752
            fprintf(stderr, "GREAT SUCCESS!\n");
         }else{
            fprintf(stderr, "What the what?? %d!=15178752\n", buffer->length);
         }
         bytes_written = bytes_to_write;//fwrite(buffer->data, 1, bytes_to_write, pData->file_handle);
         
          ros::Time scan_time = ros::Time::now();
          sensor_msgs::Image rgb_image;
          rgb_image.height=1944;
          rgb_image.width=2592;
          rgb_image.header.stamp = scan_time;
          rgb_image.header.frame_id = "rgb_optical_frame";
          rgb_image.encoding="bgr8";
          rgb_image.step=3*rgb_image.width;
          rgb_image.data.resize(rgb_image.height*rgb_image.step);
          //std::cout << rgb_image.data.size() << "," << data.colorMap.size() << std::endl;
          for(int i=0;i<rgb_image.data.size();i++){
              rgb_image.data[i]=buffer->data[i];
          }
...
{
   /// Initialize ROS
   ros::init (argc, argv, "raspicam_stills_node");
   ros::NodeHandle nh;

   image_transport::ImageTransport it_rgb(nh);
   pub_rgb_image = it_rgb.advertise("rgb/image_rect", 1);
      ...
      if (status == MMAL_SUCCESS)
      {
         ...
         while (keep_looping)
         {
            ...
            if (output_file)
            {
               ...
               if ((mmal_port_parameter_set_uint32(state.camera_component->control, MMAL_PARAMETER_SHUTTER_SPEED, state.camera_parameters.shutter_speed) != MMAL_SUCCESS))
