...
BasicLightingTechnique::BasicLightingTechnique(bool use_texture)
{   
    m_bUseTexture=use_texture;
}

bool BasicLightingTechnique::Init(std::string shader_path)
{
    if (!Technique::Init()) {
        return false;
    }
    ...
    if(m_bUseTexture){
        if (!AddShader(GL_VERTEX_SHADER, (shader_path+"basic_lighting.vs").c_str())) {
            return false;
        }


        if (!AddShader(GL_FRAGMENT_SHADER, (shader_path+"basic_lighting.fs").c_str())) {
            return false;
        }
    }else{
        if (!AddShader(GL_VERTEX_SHADER, (shader_path+"basic_lighting_rgb.vs").c_str())) {
            return false;
        }


        if (!AddShader(GL_FRAGMENT_SHADER, (shader_path+"basic_lighting_rgb.fs").c_str())) {
            return false;
        }
    }
    ...
    if(m_bUseTexture && m_colorTextureLocation == INVALID_UNIFORM_LOCATION){
        return false;
    }
